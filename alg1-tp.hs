...
-- Da la cantidad de filas de un tablero.
cantidadFilas :: Tablero -> Integer
cantidadFilas t = fromIntegral (length t)

-- Da la cantidad de columnas de un tablero.
...
caminoMasLargo [c] = c
caminoMasLargo (c:cs) | length c >= length (caminoMasLargo cs) = c
...
secuenciasDeFibonacciDeLongitudK t k = sacarDuplicados (numerosDeMuchosCaminos t (eliminarCaminosRepetidos (permutarCaminosKLista k (descartaLasMenores k (todosLosFibonacci t)))))
...
-- Elimina las listas iguales de secuenciasDeFibonacciDeLongitudK, ya que aunque hallamos eliminado los caminos iguales antes se tiene que varios caminos dan las mismas secuencias de Fibonacci.

sacarDuplicados :: [[Integer]] -> [[Integer]]
sacarDuplicados [] = []
sacarDuplicados (x:xs) | elem x xs = sacarDuplicados xs
                       | otherwise = x : sacarDuplicados xs
